...
@Component({
  selector: 'naw-reg-service-seeker',
  templateUrl: './reg-service-seeker.component.html',
  styleUrls: ['./reg-service-seeker.component.css']
})
export class RegServiceSeekerComponent {
  ...
  constructor(
    ...
    this.personalSignupForm = this.builder.group({
  ...
  onPersonalSignupSubmit() {
    const formData:any = this.personalSignupForm?.getRawValue();
    const formBody = {
      name: formData.name,
    ...
    if (this.personalSignupForm.valid) {
      // const { name, phone, email, password, confirmPassword } = this.personalSignupForm.value;
      this.authService.signupPersonal(formBody).subscribe({
        next: (response) => {
      ...
      this.personalSignupForm.markAllAsTouched();
      return;
    }
  }
  ...
  onCompanySignupSubmit() {
    ...
    if (this.companySignupForm.valid) {
      this.authService.signupFactory(formBody).subscribe({
        next: (response) => {
          this.authService.sendOtp(this.user.phone).subscribe({
            next: (response) => {
              console.log(response);
