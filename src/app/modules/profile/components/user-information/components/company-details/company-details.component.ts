...
export class CompanyDetailsComponent {
  ...
  submit() {
    const formData = this.companyDetails.getRawValue();
    const detailsBody = {
      name: formData.name,
    ...
    if (this.companyDetails.valid && imageFile) {
      const imageFormData = new FormData();
      imageFormData.append('image', imageFile);

      this.profileService.editCompanyImages(imageFormData).subscribe({
        next: (response) => {
          console.log(response);
          if (response.success) {
            this.profileService.editCompanyDetails(detailsBody).subscribe({
              next: (response) => {
                console.log(response);
